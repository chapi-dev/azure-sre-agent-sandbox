...
A comprehensive collection of prompts for Azure SRE Agent, organized by SRE discipline. Use these during demos, day-to-day operations, or to explore what SRE Agent can do in the **Movistar BSS** demo lab.
...
| Prompt | When to Use |
|--------|-------------|
| "Something is wrong with Mi Movistar. Can you investigate?" | Great starting point — lets SRE Agent discover issues on its own |
| "Are there any unhealthy resources in my resource group?" | Broad sweep across all Azure resources, not just AKS |
| "Which pods in the movistar namespace are not in a Running state?" | Quick status check across all workloads |
| "Show me all warning and error events in the movistar namespace from the last 30 minutes" | Event-level triage when you know something recently broke |
...
| "What's the exit code and termination reason for the last failed container in activation-service?" | Precise failure details |
| "Are any init containers failing across my deployments?" | Init container issues are easy to miss |
| "Compare the running pod spec for activation-service to its deployment spec — are there any drift issues?" | Detect manual overrides or config drift |

### Networking Issues

| Prompt | When to Use |
|--------|-------------|
| "Can customer-portal reach activation-service on port 3000?" | Validate service-to-service connectivity |
| "Are there any network policies blocking traffic in the movistar namespace?" | Surface overly restrictive policies |
| "Why does the activation-service Service have zero endpoints?" | Diagnose selector/label mismatches |
| "Show me DNS resolution results for subscriber-db inside the cluster" | Troubleshoot service discovery |
| "Trace the network path from customer-portal to the backend services" | End-to-end connectivity map |
...
| "Check if the subscriber-db data volume is running low on space" | Proactive disk pressure detection |
...
| Prompt | When to Use |
|--------|-------------|
| "What's the current CPU and memory utilization for each pod in the movistar namespace?" | Real-time resource snapshot |
| "Show me the request latency trends for customer-portal over the last 6 hours" | Application-level performance |
...
| Prompt | When to Use |
|--------|-------------|
| "Query the last 50 error-level logs from the activation-service container" | Targeted log retrieval |
...
| Prompt | When to Use |
|--------|-------------|
| "I just got paged — Mi Movistar is down. What's happening?" | Incident triage starting point |
...
| "How many subscribers are impacted based on the traffic patterns?" | User impact estimation |
...
| "Would scaling catalog-service to 5 replicas fit on the current nodes?" | Pre-scale feasibility check |
...
| Prompt | When to Use |
|--------|-------------|
| "Are any containers running as root in the movistar namespace?" | Security posture check |
...
| "Show me the RBAC roles and bindings in the movistar namespace" | Access control review |
...
| "Check if network policies exist for all services in the movistar namespace" | Network segmentation audit |
...
| "Show me the rollout history for activation-service" | Deployment version tracking |
| "Is the latest rollout for catalog-service complete and healthy?" | Rollout status check |
| "What image versions are running vs. what's defined in the deployment spec?" | Image version drift |
| "Compare the current activation-service deployment to the previous revision" | Diff between revisions |
...
| "Analyze the request throughput for customer-portal — is it within normal range?" | Traffic anomaly detection |
...
| "Mi Movistar is slow. Identify the bottleneck — is it CPU, memory, network, or disk?" | Performance triage |
...
| Prompt | When to Use |
|--------|-------------|
| "Map out the service dependencies in the movistar namespace" | Service topology discovery |
| "Is subscriber-db healthy and accepting connections?" | Database dependency check |
| "Is provisioning-queue running and are queues being consumed?" | Message broker health |
| "Which services depend on subscriber-db and how would they be affected if it went down?" | Blast-radius analysis |
...
| Prompt | What It Does |
|--------|-------------|
| "Restart the activation-service pods" | Rolling restart of a deployment |
| "Scale catalog-service to 3 replicas" | Horizontal scaling |
| "Delete the cpu-stress-test deployment" | Remove rogue or test workloads |
| "Remove the deny-activation-service network policy" | Unblock network traffic |
| "Scale the subscriber-db deployment back to 1 replica" | Restore a dependency |
| "Roll back the activation-service deployment to the previous revision" | Deployment rollback |
| "Cordon the unhealthy node and drain its workloads" | Node maintenance |
| "Increase the memory limit for activation-service to 256Mi" | Resource limit adjustment |
...
| "Monitor pod restarts in the movistar namespace — alert if any pod restarts more than 3 times in an hour" | Every 15 min | Restart anomaly detection |
...
| Prompt | Why It Works |
|--------|-------------|
| "Mi Movistar is broken" | SRE Agent discovers what "broken" means |
| "Customers say new line activations are slow" | Business language → technical diagnosis |
| "The CSR team can't see subscriber data" | Maps a back-office symptom to the data tier |
| "Is my platform ready for a big prepaid campaign tomorrow?" | Capacity + health in business terms |
| "Help me understand why my costs went up this month" | Bridges SRE and FinOps |
| "I'm new to this cluster — give me a tour" | Onboarding and knowledge transfer |
...
1. **Layer your prompts** — start broad ("what's wrong?"), then narrow ("why is activation-service failing?"), then act ("restart activation-service")
2. **Use business language** — SRE Agent can translate "líneas no activan" or "saldo no se actualiza" into traces, logs, and pod state
3. **Ask why, not just what** — "Why is this pod restarting?" yields better insight than "show me pod status"
